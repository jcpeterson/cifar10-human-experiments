...
class CIFAR10H(data.Dataset):
    ...
    def __init__(self, root, which_set='train',
        ...
        if self.set in ['train','test']:
            ...
            eps =  0.000001
            temp = 2.0
            for yi in range(self.c10h_targets.shape[0]):
                self.c10h_targets[yi] = (self.c10h_targets[yi] + eps) / (self.c10h_targets[yi] + eps).sum()
                self.c10h_targets[yi] = np.exp(np.log(self.c10h_targets[yi])/temp) / np.exp(np.log(self.c10h_targets[yi])/temp).sum()
