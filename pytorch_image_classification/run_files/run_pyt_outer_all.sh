...
for model in $models
  ...
  do
    ...
    sbatch --time=${time} --job-name=${model}.${held_out} --output=${log_file} --export=held_out=${held_out},out_dir=${out_dir},python_args="${python_args}",log_file=${log_file},option=$option run_pyt_inner.sh
  done
  echo 'done outer'
  done
